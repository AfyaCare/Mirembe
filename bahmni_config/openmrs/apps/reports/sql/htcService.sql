SELECT pi.identifier as PatientID,
       ...
       concat(pn.given_name ," ", pn.family_name) as 'Jina la Mshahuri Nasaha',
       ...
       AS
       `Rufaa ya Kwenda`,
         MAX( IF(conp.name = 5096, conp.value, NULL))
       AS
       `Amepewa Kondomu`,
...
FROM   visit v
	...
	LEFT OUTER JOIN person_attribute pea
         ...
         where v.location_id in (33,137) and CAST(v.date_started AS DATE) BETWEEN '#startDate#' AND '#endDate#'
