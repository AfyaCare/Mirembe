...
angular.module('bahmni.report').controller('reportsController', ['$scope',
  '$rootScope', '$stateParams', '$state', '$q', 'orderTypeService', 'sessionService', 'encounterService',
  'spinner', 'messagingService', 'appService', '$anchorScroll',
  'contextChangeHandler', '$bahmniCookieStore', 'ngDialog', 'getSaleOrdersService', '$filter', '$window',
  function($scope, $rootScope, $stateParams, $state, $q,
    ...
    var getReports = function() {
      spinner.forPromise(
        getSaleOrdersService.readReports().then(function(response) {
    ...
    $scope.fetchReport = function(report_name, time_period, year, sqlPath, period_type) {
      ...
      spinner.forPromise(
        getSaleOrdersService.requestReport(report_name, time_period, sqlPath, period_type).then(function(response) {
    ...
    $scope.previewReport = function(name, startDate, type) {
      ...
      // $window.location.href = "https://192.168.1.2/bahmnireports/report"+params; //this is for testing
