"use strict";
angular.module('bahmni.cashier').controller('cancel_ordersController', ['$scope', '$rootScope', '$stateParams', '$state', '$q', 'orderTypeService', 'sessionService', 'encounterService', 'spinner', 'messagingService', 'appService', '$anchorScroll', 'contextChangeHandler', '$bahmniCookieStore', 'ngDialog', 'getSaleOrdersService',
  function($scope, $rootScope, $stateParams, $state, $q, orderTypeService, sessionService, encounterService, spinner, messagingService, appService, $anchorScroll, contextChangeHandler, $bahmniCookieStore, ngDialog, getSaleOrdersService) {
    ...
    var getCancelOrders = function() {
      var search = '';
      spinner.forPromise(getSaleOrdersService.getCancelledorders(search).then(function(response) {
    ...
    $scope.search = function() {
      spinner.forPromise(getSaleOrdersService.getCancelledorders($scope.searchName).then(function(response) {
        console.log(response.data);
        $scope.orders = response.data;
      }));
    };
    $scope.getOrderLines = function(oder_id, fullName, discount) {
      ...
      spinner.forPromise(getSaleOrdersService.getSalesOrders_line_other(oder_id).then(function(response) {
