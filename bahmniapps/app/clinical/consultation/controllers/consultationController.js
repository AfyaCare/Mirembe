...
angular.module('bahmni.clinical').controller('ConsultationController', ['$scope', '$rootScope', '$state', '$location', 'clinicalAppConfigService', 'diagnosisService', 'urlHelper', 'contextChangeHandler',
    ...
    'ngDialog', '$filter', 'configurations', 'offlineService', 'visitConfig', 'conditionsService', 'integrationService','drugService','billingService',
    function ($scope, $rootScope, $state, $location, clinicalAppConfigService, diagnosisService, urlHelper, contextChangeHandler,
        ...
        $scope.gothomisIntegrationStatus = appService.getAppDescriptor().getConfigValue("gothomisIntegrationStatus");
        ...
        $scope.displayConfirmationDialog = function (event) {
            if ($rootScope.hasVisitedConsultation && $scope.showSaveConfirmDialogConfig) {
                ...
                ngDialog.openConfirm({
                    template: 'consultation/views/saveConfirmation.html',
                    scope: $scope
                });
            }
        };
        ...
        var loadServiceMap = function() {
            return spinner.forPromise(billingService.getServiceTypeMap())
                .then(function(response){
                    $scope.serviceTypeMap = response.data;
                    console.log(angular.toJson($scope.serviceTypeMap));
                });
        };
        ...
        var preSavePromise = function () {
            ...
            //console.log("PreSavePromise [tempConsultation]: "+angular.toJson(tempConsultation));
            addFormObservations(tempConsultation);

            $scope.orders = tempConsultation.orders;
            storeTemplatePreference(selectedObsTemplate);

            console.log("[ Consultation controller ] All orderables nje: "+angular.toJson($rootScope.allOrderableItems));
        ...
        var submitTreatmentOrderToGothomis = function (providerUuid, DrugOrder, patientUuid, locationUuid) {
        ...
        var processStockReduction = function (observations){
        var num = 0;
          var drugsTOreduct = {};
        ...
        $scope.quotationJSON = {};
        $scope.quotationJSON.items = [];
        var submitItemsForQuotation = function(patientUuid,){
            $scope.quotationJSON.patientUuid = patientUuid;
            $scope.quotationJSON.visitUuid = $scope.visitHistory.activeVisit.uuid;
            $scope.quotationJSON.payment = {};
            return spinner.forPromise(billingService.submitQuotation($scope.quotationJSON)).then(function(response){
        ...
        $scope.save = function (toStateConfig) {
            ...
            return spinner.forPromise($q.all([preSavePromise(), encounterService.getEncounterType($state.params.programUuid, sessionService.getLoginLocationUuid())]).then(function (results) {
                ...
                /*console.log("Visit history: "+angular.toJson($scope.visitHistory.activeVisit));*/
                /*console.log("Encounter data ni: "+ angular.toJson(encounterData));*/

                //Laboratory and radiology orders
                if(encounterData.orders.length>0) {
                    console.log("orders kubwa kwa zero: "+ angular.toJson($scope.orders));
                    _.forEach($scope.orders, function(order) {
                        _.forEach($rootScope.allOrderableItems, function(orderableItemSetLevel1) {
                            if(orderableItemSetLevel1.set===true) {
                                _.forEach(orderableItemSetLevel1.setMembers, function(orderableItemSetLevel2){
                                    if(orderableItemSetLevel2.set===true) {
                                        _.forEach(orderableItemSetLevel2.setMembers, function(orderableItemSetLevel3) {
                                            var conceptClassUuid = orderableItemSetLevel3.conceptClass.uuid;
                                            if(order.concept.uuid === orderableItemSetLevel3.uuid) {
                                                var concept = {};
                                                concept.uuid = order.concept.uuid;
                                                concept.name = order.concept.name;
                                                concept.class = {};
                                                concept.class.uuid = conceptClassUuid;
                                                concept.class.name = orderableItemSetLevel3.conceptClass.name;
                                                $scope.ordersToQuote.push(concept);
                                            }
                                        });
                                    }
                                });
                    ...
                    console.log("Modified orders object: "+angular.toJson($scope.ordersToQuote));
                }
