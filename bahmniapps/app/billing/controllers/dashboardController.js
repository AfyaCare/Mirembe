...
var billingApp = angular.module('bahmni.billing');
billingApp.controller('dashboardController', ['$scope', '$state', 'appService', 'locationService', 'spinner', '$bahmniCookieStore', '$window', '$q',
                ...
                angular.forEach(test.locations, function(value, key) {
                    if (value.location == getCurrentLocation().name) {
                        angular.forEach($scope.appExtensions, function(extension, key) {
                            ...
                            }else if(extension.id == "bahmni.programs" && value.app=="program")
                            {
                             extension.translationKey = value.clinicName;
                            }
